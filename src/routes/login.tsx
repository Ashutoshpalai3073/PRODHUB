...
function LoginPage() {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.case === 'google_conflict') {
          setError('This email is registered with Google login. Please use the Continue with Google button to log in.');
        } else {
  ...
  const handleOTPSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/verify-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email.trim().toLowerCase(), code: otp.trim(), type: 'login' }),
        credentials: 'include',
      });
    ...
    <div style={{ minHeight: '100vh', background: '#020208', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ background: '#0a0a14', border: '1px solid rgba(139,92,246,0.25)', borderTop: '2px solid #8b5cf6', borderRadius: 20, padding: '32px 28px', boxShadow: '0 24px 64px rgba(0,0,0,0.6)', position: 'relative' }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, color: 'white', margin: '0 0 6px' }}>Log In</h1>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', margin: '0 0 28px' }}>
            {step === 'email' ? 'Enter your email to receive a one-time code.' : `Enter the 6-digit code sent to ${email}`}
          </p>
          ...
          {error && (
            <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.25)', color: '#fca5a5', fontSize: 13, marginBottom: 16, lineHeight: 1.5 }}>
              ...
              {error.includes('sign up') && (
                <Link to="/signup" style={{ color: '#a78bfa', marginLeft: 6, fontWeight: 600 }}>Sign Up</Link>
              )}
            </div>
          )}
          ...
          {step === 'email' ? (
            <form onSubmit={handleEmailSubmit}>
              ...
              <button type="button" onClick={() => loginWithGoogle('login')} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '11px 20px', borderRadius: 10, background: 'white', border: 'none', cursor: 'pointer', fontSize: 14, fontWeight: 600, color: '#1a1a2e' }}>
                <GoogleIcon />
            ...
            <form onSubmit={handleOTPSubmit}>
              ...
              <button type="button" onClick={() => { setStep('email'); setOtp(''); setError(''); }} style={{ width: '100%', padding: '10px', background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', fontSize: 13, cursor: 'pointer' }}>
                ← Back to email
              </button>
            </form>
          )}
